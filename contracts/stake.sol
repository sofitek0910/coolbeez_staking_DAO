...
contract StakeToken is ERC20 {
  ...
  struct PoolInfo {
    ...
    uint256[4] alloc; // the reward amount per day for common NFT, rare NFT, queenNFT, hyperNFT
    uint256[4] totalKindNFT; // the total amount of common nft, rare NFT, queenNFT, hyperNFT
  }
  ...
  struct StakerInfo {
    ...
    uint256[4] kindNFT; // the common NFT count, rare NFT, queen NFT, hyper NFT
   ...
   * @param _alloc the reward amount per day for common NFT, rare NFT, queen nft, hyper nft
  ...
  function stakeTokens(uint256[] memory _tokenIds, uint256[4] memory kindNFT) public {
    require(
    ...
    _staker.kindNFT[0] = _staker.kindNFT[0].add(kindNFT[0]); //common nft
    _staker.kindNFT[1] = _staker.kindNFT[1].add(kindNFT[1]); //rare nft
    _staker.kindNFT[2] = _staker.kindNFT[2].add(kindNFT[2]); //queen nft
    _staker.kindNFT[3] = _staker.kindNFT[3].add(kindNFT[3]); //hyper nft
  ...
  function getLastStakableBlock() public view returns (bool) {
    ...
    uint256 _rewardCommonAmount = pool.alloc[0].mul(pool.totalKindNFT[0]).mul(1e36);
    _rewardCommonAmount = _rewardCommonAmount.div(86400);
    uint256 _rewardRareAmount = pool.alloc[1].mul(pool.totalKindNFT[1]).mul(1e36);
    _rewardRareAmount = _rewardRareAmount.div(86400);
    uint256 _rewardQueenAmount = pool.alloc[2].mul(pool.totalKindNFT[2]).mul(1e36);
    _rewardQueenAmount = _rewardQueenAmount.div(86400);
    uint256 _rewardHyperAmount = pool.alloc[3].mul(pool.totalKindNFT[3]).mul(1e36);
    _rewardHyperAmount = _rewardHyperAmount.div(86400);

    uint256 _rewardAmount = _rewardCommonAmount.add(_rewardRareAmount);
    _rewardAmount = _rewardAmount.add(_rewardQueenAmount);
    _rewardAmount = _rewardAmount.add(_rewardHyperAmount);
  ...
  function calcHarvestTot(address _userAddy) public view returns (uint256) {
    ...
    uint256 _rewardCommonAmount = pool.alloc[0].mul(_staker.kindNFT[0]).mul(_nrOfBlocks).mul(1e36);
    _rewardCommonAmount = _rewardCommonAmount.div(86400);
    uint256 _rewardRareAmount = pool.alloc[1].mul(_staker.kindNFT[1]).mul(_nrOfBlocks).mul(1e36);
    _rewardRareAmount = _rewardRareAmount.div(86400);
    uint256 _rewardQueenAmount = pool.alloc[2].mul(_staker.kindNFT[2]).mul(_nrOfBlocks).mul(1e36);
    _rewardQueenAmount = _rewardQueenAmount.div(86400);
    uint256 _rewardHyperAmount = pool.alloc[3].mul(_staker.kindNFT[3]).mul(_nrOfBlocks).mul(1e36);
    _rewardHyperAmount = _rewardHyperAmount.div(86400);
