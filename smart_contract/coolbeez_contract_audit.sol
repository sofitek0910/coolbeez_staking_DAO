...
contract coolbeez is ERC721Enumerable, Ownable {
    ...
    */
    ...
    /** Add multiple addresses to whitelist */
    function addAddressesToFreeMintWhiteList(address[] memory addresses) public onlyOwner {
        for(uint256 i =0; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Address can not be zero");
            require(!whitelist[addresses[i]], "This address is already registered");
            freeMintWhitelist[addresses[i]] = true;
        }
    }
